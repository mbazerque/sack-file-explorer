...
function App() {
  const { activeTab, createTerminalTab, setActivePanel, toggleSplitView } = useTabContext();
        ...
        <div className="flex-1 flex min-h-0 min-w-0 overflow-hidden">
          ...
          <main className="flex-1 overflow-y-auto bg-gray-950 relative min-w-0 flex flex-col">
            {activeTab.type === "terminal" ? (
              <TabTerminal
                sessionId={activeTab.terminalId || `session-${activeTab.id}`}
                cwd={activeTab.terminalPath || activeNav.currentPath}
              />
            ) : !isSplitViewOpen ? (
              // Single Panel View
              <FileList
